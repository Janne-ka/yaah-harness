...
Agents are parse-by-default ([ADR-0004](../../../docs/decisions/0004-parse-by-default.md)) and the model's JSON reply **becomes the payload** (`raw` + parsed keys + `carry:` keys — everything else is gone). `render` and `branch` find the model's keys without an intermediate `transform`, but any PRE-agent key (e.g. an `input` field) that a LATER stage still reads must be listed in the agent's `"carry": [...]`, or that stage fails at runtime with `render_unfilled_placeholders`. `validate --strict` can NOT pre-catch the missing-carry case (a parsing agent counts as "could provide anything") — trace it by hand per check 6 of the pre-handoff walk. The `"parse": false` opt-out IS caught at load time by `validate.py`.
...
4. **Providers?** *Suggest:* `claude_cli` for real, `fake_scripted` for the fake overlay. Always build both. Other backends in `adapters/providers/` (catalog).
