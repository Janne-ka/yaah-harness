...
- **fanout: [roles]** → the role BARRIER: run those roles in parallel, merge to `{results, roles, failed_roles}`, continue (review's 7 lenses; the sceptic bank). A member that raises, replies `Kind.ERROR`, or RETURNS a failed verdict (an agent exhausting its schema/parse attempts) is a **failed role**; by default any failed role fails the stage (`fanout_error`, naming each member's own failure). **`min_success: k`** opts into k-of-n completion: with ≥ k healthy members the stage PASSES the merged payload — `failed_roles` still names the dead ones, so a downstream reducer can emit its degraded-mode concern instead of the whole stage dying for one flaky lens.
- **foreach: {items, into?, carry?, max_concurrent?}** (ADR-0007) → dynamic per-item fan-out: run this stage's ONE `node` once per element of `payload[items]` — a list whose size is only known at runtime (one skeptic per extracted requirement) — at most `max_concurrent` (default 3) in flight. Each item's input is REPLACE + named carries, never a copy of the whole inbound: `{into: element, item_index, carry keys, graph.sticky keys}` — a 50-KB document only rides along if you `carry` it (the visible-cost rule). Merge: inbound payload ∪ `{results: [{item_index, payload}…] in item order, failed_items: [indexes]}` — pairs, because a reset worker (a plain agent) drops `item_index` from its own output and a compacted bare list would misalign after failures. Failure classification and `min_success: k` are exactly fanout's (shared code); the k floor also holds with zero failures (an empty/short list can under-deliver). v1 limits, documented: an item replying AWAIT parks the WHOLE stage; a stage retry re-runs ALL items; execution is in-memory (use fork/fanin for a durable join); `feedback:true` keys are not threaded per item.
- **fork: [stages]** → spread the same envelope to those successor STAGES; each runs as an independent concurrent branch with its own continuation. Branches may end on their own or rejoin at a fan-in. (Explicit key — distinct from `fanout`, the one-stage barrier over ROLES; the two used to share the `fanout` key with the meaning sniffed from the targets, split 2026-06-11.) A fork with a `then` (or a `wait`) is a **synchronized scatter-gather**: it waits for a **`clear`** signal addressed **`<node-id>:<correlation_id>`** (the gate's configurable unique `id`, default the stage name, paired with the run id) over the `clear` topic, then resumes the forking flow carrying that result. The address rides each branch envelope (header `clear_id`, preserved through replies) to the fan-in, which echoes it on the clear. The clear is **sender-agnostic**: the fan-in, a human, a timer, or any party that knows the gate + run may publish it; the fork matches on the address, not the sender (so it works in-proc and cross-process unchanged). Optional `wait: {timeout, on_timeout, clear_topic}` bounds the wait; on degrade the fork publishes to the listener, abandons the branches, and proceeds with whatever the join had already collected (**`fork_partial`**, below). The `timeout` is a pure LIVENESS bound: when the outcome is already determined — an arm died (`reason: branch_failed`, with the error in `detail`) or every arm settled with the join provably unmeetable (`reason: fanin_unmeetable`) — the degrade happens immediately instead of sitting out the TTL; only a clean settle still awaiting a possible external clear runs to the TTL (`reason: wait_timeout`). A fork with no `then`/`wait` is terminal — the fan-in's own `then` carries the result (the decoupled a/b/c/d case).

  **`fork_partial` — the degraded fork's output (reserved payload key).** On any of the three degrade routes above (`wait_timeout` / `branch_failed` / `fanin_unmeetable`), an arm that had ALREADY deposited at the fan-in is real, paid-for work; returning the pre-fork payload would throw it away and read downstream as "the fork produced nothing", which is a lie. So the continuation runs on the inbound payload **plus** one reserved key:
  ...
  - **`expected`/`missing` name branches whenever they are knowable.** A list `expect` names them outright. A `{"count": n}` expect names none — *unless* `n` equals the fork's width, which means "every arm", and branch ids ARE the fork's stage names; then the fork's own list is used. A shorter count, or an omitted `expect` ("whatever arrives"), leaves both empty: a partial policy does not say which arms it wanted, and the engine will not invent ids.
  - **The fan-in's `reduce` is deliberately NOT run.** `reduce` is declared for a MET policy; running it over a partial set would fabricate a full-fork result that nothing downstream could distinguish from the real thing. The engine hands over the raw arrivals and takes no view of their shape (same rule as `fanout`'s `min_success` degrade: pass the healthy results forward with a marker naming what is missing, never silently discard or silently complete).
  - **The parked sets are flushed** as on every other end-of-join path — the data now rides the payload, so the store must not keep a second copy.
  - **Ordering: the fan-in may release its parked set BEFORE the fork degrades, and the salvage survives it anyway.** Two of the three degrade routes are *decided by the fan-in itself*: on `branch_failed` and `fanin_unmeetable` the join is released as unmeetable and its coordinator exits — publishing `on_timeout`, dropping the parked set — strictly before the fork observes the outcome. A fan-in whose OWN `fanin.timeout` is shorter than the fork's `wait.timeout` does the same, earlier, on its own clock (the reason the fork then reports is still its own `wait_timeout` — the fan-in gave up early, the fork ran out later). Every such exit therefore **snapshots the arrivals onto the join before flushing**, and the degrade reads the snapshot when the live store listing is empty. Without it the salvage would exist on `wait_timeout` alone, which is how it originally shipped.
  - **A broken store costs the salvage and nothing more.** The drain is guarded: if the EnvelopeStore cannot be listed (full disk, swept file, dead backend), the fault is traced as a `fork_partial_salvage_failed` error span and the degrade returns the pre-fork envelope — the behaviour it had before the salvage existed. Degrading must never be worse than not degrading.
  - **A multi-join fork FLATTENS.** `results` is one `{branch_id: payload}` map across every fan-in in the fork, and `expected` is the union — so two joins reached by the same branch id would collide (last wins). Single-join forks (every shipped one) are unaffected; a per-join shape would change this published contract for a case nobody has.
  - **Consumer obligation.** `fork_partial.results` holds whole branch payloads, which are usually the largest objects in the run. A consumer that salvages them (an app transform reading `fork_partial` and emitting its degraded-mode concern) should **remove the large arm payloads from its own output** — otherwise they ride the envelope through every remaining stage and into every prompt, at cost. Read them, reduce them to what the continuation needs, and drop the key.
