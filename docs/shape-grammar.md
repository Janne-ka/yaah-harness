...
| `strict_render: true` | fail the stage (`render_unfilled_placeholders`) on a `{{placeholder}}` with no value in payload ∪ extras, instead of leaving the literal `{{name}}` (default `false`); engine-injected keys + present-but-empty values never trip it, and a placeholder marked optional as `{{?key}}` renders empty instead of faulting (for loop-seeded keys like `{{?loop_feedback}}`) |
