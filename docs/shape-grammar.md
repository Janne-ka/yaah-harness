...
YAAH ships JSON Schemas for both root and pipeline files. Wire them
in your editor and you get autocomplete on every key, enum, and node
type — and red squiggles on typos before the runtime ever loads.

Add `$schema` at the top of your config:

```jsonc
// in a root config:
{
  "$schema": "<path-or-url>/schemas/root.schema.json",
  ...
}

// in a pipeline config:
{
  "$schema": "<path-or-url>/schemas/pipeline.schema.json",
  ...
}
```

Path resolution depends on where YAAH is installed:

- **Source checkout / submodule:** relative path from your config to
  `schemas/root.schema.json` in the yaah repo
  (e.g. `"$schema": "../../yaah/schemas/root.schema.json"`).
- **pip install:** the schemas ship in the wheel under
  `site-packages/yaah/schemas/` — use that absolute path, OR symlink
  into your project, OR pin a URL once we publish (TODO).

The schemas are regenerated by `scripts/build_schemas.py` from the
engine's own validation tables (`src/yaah/validate.py` +
`src/yaah/runtime_factories.py`). A test in the suite asserts the
committed schemas match what the generator produces — drift is
caught at suite time, not when a user complains the autocomplete
lies.
